...
class SocialSignupController extends GetxController{
  ...
  ///*
  ///
  /// navigate to SocialOtpVerificationScreen
  navigateToOtpScreen() {
  ...
  ///*
  ///
  /// hit social_otp/ Api to get otp on Email
      ...
      if(results != null){
        ...
        if(responseModel.status == 200){
          if(responseModel.payload != null){
            ...
            //get otp if email not register for vendor nor customer
            if(responseModel.payload!.otp != 0){
              ...
              clearTextField();
              navigateToOtpScreen(); //navigate to otp screen for otp verification
            }else{
              //you will get 0 in otp param if email will have registered either for vendor or customer
              ...
              //show ask dialog with message
              // if email already register as vendor then show message
              // This email is already registered as Vendor. Do you want to continue as customer? if yes then email register for both role
              // vice versa if email already registered as Customer
              showDialog(
                  context: Get.context!,
                  builder: (BuildContext context1) => AskDialog(
                      my_context: Get.context!,
                      msg: responseModel.payload!.signupMsg!,
                      yesFunction: yesFunction, //check this function
  ...
  /// if email already registered as vendor and user agree to become customer with same email
  /// then current userRole for app will be Customer
  /// else if email already registered as Customer and user agree to become Vendor with same email
  /// then current userRole for app will be Vendor
  /// and last call signup api
  yesFunction() {
  ...
  ///*
  ///
  /// update user with below data using social_signup/ Api on server
      ...
      if (results != null) {
        ...
        if (responseModel.status == 200) {
          if (responseModel.payload != null) {
            ...
            updateFirebaseUser(); //once user update Data on Server also Update Data against same user into firebase
          }
        } else { // if error occur then msg is "Something went wrong or validation msg"
          showDialog(
            context: Get.context!,

...
class _CustomerCheckoutScreenState extends State<CustomerCheckoutScreen> {
  ...
  void setBookOrderRequestBody() {
    ...
    _getXController.requestModel.bookingTime = widget.bookTime;
    _getXController.requestModel.bookingStartTime = widget.bookTime;
    _getXController.requestModel.bookingEndTime = widget.bookTime;

    DateTime time = DateTime.parse(widget.bookDate + " " + widget.bookTime!);
    var endTime = time.add(new Duration(hours: 3));  //api changes required in cartdata api calculation avg time. repace with total time
